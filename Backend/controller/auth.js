...
class AuthController {
    static async userRegistration(req, res) {
        ...
        try {
            let { name, email, dept, password } = req.value.body;
            ...
            UserModel.findOne({ email }, async function (err, user) {
                ...
                let userData = await UserController.create(name, email, password, dept, verifyEmailToken);
    ...
    static async userPriv(req, res) {
        try {
            UserModel.findOne({ email: req.body.email }, async function (err, uzer) {
                if (!uzer) {
                    return res.status(404).json({ success: false })
                }
                uzer.title = req.body.title;
                uzer.save()
            });
