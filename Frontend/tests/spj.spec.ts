...
test.describe("Testing menggunakan akun divisi PSDM", () => {
  ...
  test("TC16 - Pengajuan SPJ Sukses", async ({ page }) => {
    await page.goto(baseurl + `spj/upload`);
    ...
    await page.goto(`${baseurl}spj`);
    const item = page.locator("tr#item");
    await expect(item).toBeDefined();
  });
  test("TC17 - Pengajuan SPJ Gagal", async ({ page }) => {
    await page.goto(baseurl + `spj/upload`);
    await page.fill("input[name=name]", "Ilkomerz Night");
...
test.describe("Testing menggunakan akun divisi BPH", () => {
  ...
  test("TC19 - Pemberian status dan komentar SPJ", async ({ page }) => {
    await page.goto(baseurl + `spj`);
    ...
    await expect(page).toHaveURL(`${baseurl}spj/revisi`);
