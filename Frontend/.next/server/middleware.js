...
/* harmony import */ var next_dist_server_web_adapter__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(384);
        ...
        var mod = __webpack_require__(578)
...
function middleware(req) {
    ...
    if (uri === "/" || uri.startsWith("/spj") || uri.startsWith("/rab") || uri.startsWith("/cashflow")) {
        if (!req.cookies.has("token")) {
            const url1 = req.nextUrl.clone();
            url1.pathname = "/auth/login";
            return server.NextResponse.redirect(url1);
        }
    }
    if (uri === "/admin" && req.cookies.has("token")) {
        const tkn = jwt_decode_esm(req.cookies.get("token"));
        if (tkn.privilege !== "pusat") {
            const url2 = req.nextUrl.clone();
            url2.pathname = "/";
            return server.NextResponse.redirect(url2);
        }
    }
